window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* The Context Dashboard's sidebar entry: a footer action ordered first in
		* the harness's footer actions row (order -1), so it renders on its own
		* line above the cost-meter balance row instead of directly above Settings,
		* mirroring the Settings trigger's geometry in both column widths. The button carries the plugin emblem and, on the wide
		...
		*/
		function makePluginInfo(kit) {
			...
			return function PluginInfo() {
				...
				const update = latest !== null && isNewerVersion(latest, "0.53.2") ? latest : null;
				const nameText = "dsh-context (v0.53.2)";
